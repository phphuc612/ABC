...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        parser_mask_changeable = (
            parse_array == LABEL_MAP["background"]
        ).astype(np.float32)
        ...
        if (
            ...
            with open(
                os.path.join(dataroot, "keypoints", pose_name), "r"
            ) as f:
                data = json.load(f)
        ...
        # elif self.args.height > 256:
        #     parse_mask = cv2.dilate(
        #       parse_mask, np.ones((10, 10), np.uint16),
        #       iterations=5
        #   )
        else:
            parse_mask = cv2.dilate(
        ...
        uv = np.load(
            os.path.join(
                dataroot, "dense", im_name.replace("_0.jpg", "_5_uv.npz")
            )
        )
        uv = uv["uv"]
